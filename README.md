...
Kotlin Multiplatform line-by-line clean-room port of the Rust crate [`tokio-tungstenite`](https://crates.io/crates/tokio-tungstenite).

> **Status: scaffold — porting has not started.** This repo currently contains build infrastructure only. The upstream Rust source for the `tokio-tungstenite` crate will be cloned into `tmp/` (gitignored) when porting begins.
...
Original work copyrighted by the upstream `tokio-tungstenite` authors. Kotlin port copyright (c) 2026 Sydney Renee and The Solace Project.
