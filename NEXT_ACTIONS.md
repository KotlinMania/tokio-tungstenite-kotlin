...
- **Provenance warning:** port-lint provenance header matched only after fallback normalization: `tests:connect.rs` vs expected `connect.rs`
- **Proposed provenance header:** `// port-lint: source connect.rs` (current: `// port-lint: source connect.rs`)
- **Proposed provenance header:** `// port-lint: tests connect.rs` (current: `// port-lint: tests connect.rs`)
- **Lint issues:** 2
